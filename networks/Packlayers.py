...
class Conv2D(nn.Module):
    ...
    stride : int
        Stride
    """
    def __init__(self, in_channels, out_channels, kernel_size, stride):  # 会做padding，只要stride=1，kernel是奇数，输出图像大小不变
        super().__init__()
...
def ResidualBlock(in_channels, out_channels, num_blocks, stride):
    """
    ...
    if stride != 1 or in_channels != out_channels * 4:
        downlayers = []
        downlayers.append(nn.Conv2d(in_channels, out_channels * 4, kernel_size=1, stride=stride, bias=False))
        downlayers.append(nn.BatchNorm2d(out_channels*4))
        downsample = nn.Sequential(*downlayers)
    layers = [Bottleneck(in_channels, out_channels, stride=stride, radix=2, downsample=downsample)]
    for i in range(1, num_blocks):
        layers.append(Bottleneck(out_channels*4, out_channels, stride=1, radix=2))
